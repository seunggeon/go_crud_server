...
func (u *userRouter) create(c *gin.Context) {
	...
	if err := c.ShouldBind(&req); err != nil {
		...
		u.router.failedResponse(c, &types.CreateUserResponse{
			ApiResponse: types.NewApiResponse("Create Error 입니다.", -1, err.Error()),
		})
	} else {
...
func (u *userRouter) update(c *gin.Context) {
	...
	if err := c.ShouldBind(&req); err != nil {
		...
		u.router.failedResponse(c, &types.UpdateUserResponse{
			ApiResponse: types.NewApiResponse("Update Error 입니다.", -1, err.Error()),
		})
	} else {
...
func (u *userRouter) delete(c *gin.Context) {
	...
	if err := c.ShouldBind(&req); err != nil {
		u.router.failedResponse(c, &types.DeleteUserResponse{
			ApiResponse: types.NewApiResponse("바인딩 오류 입니다.", -1, nil),
		})
	} else if err = u.userService.Delete(req.ToUser()); err != nil {
		u.router.failedResponse(c, &types.DeleteUserResponse{
			ApiResponse: types.NewApiResponse("Delete Error 입니다.", -1, err.Error()),
		})
	} else {
